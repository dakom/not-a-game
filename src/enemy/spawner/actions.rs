...
use crate::{animation::data::Animation, collision::data::Collider, config::CONFIG, dom::{ui::UiPhase, DomView}, enemy::{controller::data::{EnemyControllerFour, EnemyControllerOne, EnemyControllerThree, EnemyControllerTwo}, data::{Enemy, EnemyFourPhase, EnemyKind, EnemyOnePhase, EnemyThreePhase, EnemyTwoPhase}, effects::data::EnemyEffect, events::EnemySelectEvent, launcher::data::{EnemyLauncher, LauncherSide}, physics::data::EnemyDirection}, layout::data::{LayoutAnchor, LayoutPosition}, media::MediaView, prelude::*, renderer::RendererViewMut, tick::{BeginTickView, UpdateTickView}};
...
pub fn spawn_enemies(world: &World) {
    world.run(| 
        ...
        // to allow more args 
        mut enemy_group: (ViewMut<LayoutPosition>, ViewMut<LayoutAnchor>,ViewMut<EnemySelectEvent>, ViewMut<EnemyEffect>),
        mut colliders: ViewMut<Collider>,
        mut renderer: RendererViewMut,
        media: MediaView
    | {
        let (mut enemy_positions, mut enemy_anchors, mut enemy_select, mut enemy_effects) = enemy_group;
        ...
        for spawn_index in 0..4 {
            ...
            (&mut animations, &mut enemies, &mut enemy_positions, &mut enemy_anchors, &mut colliders, &mut enemy_effects).add_component_unchecked(entity, 
                (
...
pub fn spawn_launcher(world: &World, side: LauncherSide) {
    ...
    | {
        ...
        (&mut animations, &mut launchers, &mut positions).add_component_unchecked(entity, 
            (
