use nalgebra::{convert, Point, Point3, Point2};
use nalgebra_glm::Vec2;

use crate::{prelude::*, enemy::data::Enemy, layout::data::{LayoutPosition, LayoutAnchor}, projectiles::data::{Projectile, ProjectileMovement}, renderer::{RendererViewMut, uvs::Uvs, buffers::Buffers}, explosion::data::{ExplosionSpawner, Explosion}, animation::data::Animation};

use super::data::{Collider, CollisionEventQueue, CollisionEvent, CollisionEventTarget, CollisionEventQueueViewMut};
...
pub fn update_collider_sys(
    ...
    for (collider, enemy, animation, transform) in (&mut collider, &enemies, &animations, &transform).iter() {
...
pub fn detect_geometric_collision_sys(
    ...
    for (entity_p, (collider_p, transform_p, projectile)) in (&colliders, &transforms, &projectiles).iter().with_id() {
        match projectile.movement {
            ProjectileMovement::Bomb { .. } => {
                bomb_projectiles.push((entity_p, collider_p, transform_p, projectile));
                for(entity_e, (collider_e, transform_e, enemy)) in (&colliders, &transforms, &enemies).iter().with_id() {
                    if enemy.controller().hiding().is_some() {
                        // enemy is hiding, skipping collision check
                        continue;
                    }
                    if !event_queue.has_any_collision(entity_p, entity_e) && collider_p.intersects_rect(collider_e) {
                        ...
                        event_queue.push(CollisionEvent{
                            ...
                            b: CollisionEventTarget { 
                                ...
                                uvs: Uvs::new(spritesheet.atlas_width, spritesheet.atlas_height, &spritesheet.cells[0]).data,
                            },
    ...
    for (entity_b, collider_b, transform_b, projectile_b) in &bomb_projectiles {
        for (entity_o, collider_o, transform_o, projectile_o) in &other_projectiles {
            if !event_queue.has_any_collision(*entity_b, *entity_o) && collider_b.intersects_rect(collider_o) {
                event_queue.push(CollisionEvent {
                    a: CollisionEventTarget {
                        entity: *entity_b,
