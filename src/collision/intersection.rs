use anyhow::Context;
use awsm_web::webgl::{FrameBufferTarget, Buffer, GlQueryKind, GlToggle, BlendFactor, BufferData, BufferTarget, BufferUsage, BeginMode, CmpFunction, DrawBuffer, BufferMask};
use nalgebra_glm::Vec2;
use crate::prelude::*;

use crate::renderer::Renderer;

use super::data::{Collider, CollisionEventTarget, CollisionEvent, CollisionEventQueue};
...
impl CollisionEvent {
    ...
    pub fn render_pixel_intersection(&mut self, renderer: &mut Renderer) -> Result<()> {
        ...
        let fbo_id = renderer.framebuffers.as_ref().map(|f| f.fbo_collision.id).context("no fbo found")?;
        ...
        renderer.gl.gl.stencil_func(CmpFunction::Always as u32, 1,0xFF);
        ...
        renderer.gl.gl.stencil_func(CmpFunction::Equal as u32, 1,0xFF);
