...
use nalgebra::{Quaternion, UnitQuaternion, convert};
use serde::de::value::MapAccessDeserializer;
use shipyard_scenegraph::traits::required::Vec3Ext;

use crate::{prelude::*, enemy::{launcher::data::{EnemyLauncher, LauncherSide}, data::Enemy, physics::data::EnemyDirection}, tick::UpdateTickView, renderer::RendererView, delete::data::MarkForDeletion, layout::data::{LayoutPosition, LayoutAnchor}, collision::data::Collider};
use super::data::{ProjectileSpawnerViewMut, Projectile, ProjectileToSpawn, ProjectileSpawner, ProjectileMovement};
...
pub fn projectile_spawn_sys(
    ...
    let ProjectileSpawner { good_rocket_height, good_rocket_width, good_rocket_texture_id, bad_rocket_width, bad_rocket_height, bad_rocket_texture_id, bullet_width, bullet_height, bullet_texture_id, .. } = *spawner;
    ...
    for to_spawn in spawner.to_spawn.drain(..) {
        match to_spawn {
            ProjectileToSpawn::Bomb { } => {
                ...
                (&mut projectiles, &mut positions, &mut anchors, &mut colliders).add_component_unchecked(entity, 
                    (
                        Projectile{
            ...
            ProjectileToSpawn::BadRocketFromGround { side } => {
                for launcher in (&launcher).iter().filter(|launcher| launcher.side == side) {
                    ...
                    let rot = UnitQuaternion::from_axis_angle(&Vec3::z_axis(), rotation.to_radians());
                    let entity = sg_storages.spawn_child_trs_origin(None, None, Some(rot.quaternion().to_owned()), Some(scale), None);
            ...
            ProjectileToSpawn::BadRocketFromEnemy {  } => {
                ...
                (&enemies, &positions).iter().for_each(|(enemy, position)| {
                    match enemy {
            ...
            ProjectileToSpawn::Bullet {  } => {
                ...
                for (pos, rotation, velocity) in to_spawn.drain(..) {
                    ...
                    let entity = sg_storages.spawn_child_trs_origin(None, None, Some(rot.quaternion().to_owned()), None, Some(origin));
                    ...
                    (&mut projectiles, &mut positions, &mut anchors, &mut colliders).add_component_unchecked(entity, 
                        (
                            Projectile{
                                ...
                                movement: ProjectileMovement::Bullet { velocity, acceleration, rotation}
                            },
                            LayoutPosition::new(pos),
                            LayoutAnchor::new(Vec3::new(if rotation > 0.0 { -60.0 } else { 60.0 }, 140.0, 0.0)),
                            Collider::default()
                        )
                    );
                }
...
pub fn projectile_physics_sys(
    ...
    for (id, (mut projectile, mut pos, mut quat)) in (&mut projectiles, &mut positions, &mut rotations).iter().with_id() {
        match &mut projectile.movement {
            ProjectileMovement::Bullet { velocity, acceleration, rotation } => {
            ...
            ProjectileMovement::Rocket { rotation, thrust_speed, rotation_speed } => {
                ...
                *quat = Rotation::new(UnitQuaternion::from_axis_angle(&Vec3::z_axis(), rotation.to_radians()).quaternion().to_owned());
            ...
            ProjectileMovement::Bomb { velocity, acceleration } => {
                pos.x += velocity.x * tick.delta as f32;
                pos.y += velocity.y * tick.delta as f32;
                pos.z += velocity.z * tick.delta as f32;
